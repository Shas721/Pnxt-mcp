...
class AuthSession:
    ...
    def get_access_token(self) -> str: return self.access_token
    def get_refresh_token(self) -> str: return self.refresh_token
    def get_tenant_id(self) -> str: return self.tenant_id
    def clear_session(self) -> None: self.access_token = self.refresh_token = self.tenant_id = ""
...
def get_session() -> AuthSession | None: return _session
...
def set_session(payload: dict[str, Any]) -> AuthSession:
    ...
    _session = AuthSession(data.get("accessToken") or data.get("access_token") or "",
                           ...
                           data.get("userId") or data.get("user_id"), data.get("email"), parsed)
    return _session
def current_user() -> dict[str, Any]:
    return _session.as_dict() if _session else {"authenticated": False, "email": None,
        "user_id": None, "tenant_id": None, "membership_id": None,
        "session_expiry": None, "expires_in_seconds": None}
